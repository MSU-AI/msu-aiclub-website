...
export default function MembersPage() {
  ...
  const handleRoleChange = (memberId: string, newRoles: string[]) => {
    handleFieldChange(memberId, 'roles', newRoles);
  };

  const handleRoleRemove = (memberId: string, roleToRemove: string) => {
    const member = members.find(m => m.id === memberId);
    if (member) {
      handleRoleChange(memberId, member.roles.filter(role => role !== roleToRemove));
    }
  };

  const handleRoleAdd = (memberId: string, newRole: string) => {
    const member = members.find(m => m.id === memberId);
    if (member && !member.roles.includes(newRole)) {
      handleRoleChange(memberId, [...member.roles, newRole]);
    }
  };
                ...
                <div className="flex flex-wrap gap-1">
                    ...
                    <Badge key={role} className={`mr-1 mb-1 ${TAG_COLORS[index % TAG_COLORS.length]}`}>
                      ...
                      {isAdmin && (
                        <X
                          ...
                          onClick={() => handleRoleRemove(member.id, role)}
                        />
                      )}
                    </Badge>
                  ))}
                  {isAdmin && (
                    <Input
                      ...
                      onKeyPress={(e) => {
                        if (e.key === 'Enter') {
                          handleRoleAdd(member.id, e.currentTarget.value);
